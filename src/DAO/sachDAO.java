...
public class sachDAO {
    ...
    public static void themSach(sachDTO s,QueryCallBack q) {
      ...
      String sql = "insert into sach (masach, tensach, tacgia, nhaxb, theloai,sl,slcon) values (?, ?, ?, ?, ?,?,?)";
      MysqlDataAccessHelper my = new MysqlDataAccessHelper();
     ...
     public static void xoaSach(String masach,QueryCallBack q) {
      ...
      PreparedStatement preparedStmt;
       ...
       public static void suaSach(sachDTO s,QueryCallBack q) {
           // create the mysql insert preparedstatement
      String query = "UPDATE sach SET tensach = ?, tacgia = ?, nhaxb = ?, theloai = ?, sl = ?, slcon = ? WHERE masach = ?";
      ...
      PreparedStatement preparedStmt;
       try {
            ...
            preparedStmt.setString(7,s.getMaSach());
            preparedStmt.execute();
            q.onSuccess();
        } catch (SQLException ex) {
            my.displayError(ex);
